...
# ------------------------------------------------------ #  
    ...
    async def unlock (self, ctx, channel=None):
        ...
        if channel:
            ...
            embed = discord.Embed(title = self.bot.translate.msg(ctx.guild.id, "unlock", "UNLOCKED_WITH_SUCCESS").format(channel.name), description = f"Channel Bigouunlock", color = 0x2fa737) # Green
