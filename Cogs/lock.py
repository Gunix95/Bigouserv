...
# ------------------------------------------------------ #  
    ...
    async def lock (self, ctx, channel=None):
        ...
        if channel:
            ...
            embed = discord.Embed(title = self.bot.translate.msg(ctx.guild.id, "lock", "LOCKED_WITH_SUCCESS").format(channel.name), description = f"Channel Bigoulocked", color = 0x2fa737) # Green
